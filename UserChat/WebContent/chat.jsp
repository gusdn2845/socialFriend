...
<%
	...
	if(toID != null){
		int result = new UserDAO().searchPerson(toID);
		if(result == 0){
			session.setAttribute("messageType", "오류 메세지");
			session.setAttribute("messageContent", "선택한 대화상대가 존재하지 않습니다.");
			response.sendRedirect("index.jsp");
			return;
		}else if(result == -1){
			session.setAttribute("messageType", "오류 메세지");
			session.setAttribute("messageContent", "오류가 발생했습니다.");
			response.sendRedirect("index.jsp");
			return;
		}
	}
	...
	if(userID.equals(URLDecoder.decode(toID,"UTF-8"))){
		session.setAttribute("messageType", "오류 메세지");
		session.setAttribute("messageContent", "자신에게는 메세지를 보낼수 없습니다.");
		response.sendRedirect("index.jsp");
		return;
	}
	
	String fromProfile = new UserDAO().getProfile(userID);//자신의 사진
	String toProfile = new UserDAO().getProfile(toID);//상대방의 사진
%>
<meta charset="UTF-8">
